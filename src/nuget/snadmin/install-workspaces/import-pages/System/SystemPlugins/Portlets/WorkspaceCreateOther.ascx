    ...
    <ul id="sn-createitem-types">
        <li class="sn-pos-1 sn-active">
            ...
            <p><%= SenseNetResourceManager.Current.GetString("Portal", "WorkspaceCreateOther_WorkspacesDescription") %></p>
                ...
                %>
                <dt><a href='<%= actionUrl %>'>
                    <%= IconHelper.RenderIconTag(ct.Icon) + SenseNet.ContentRepository.Content.Create(ct).DisplayName%></a> </dt>
                <dd>
                    <%= SenseNet.ContentRepository.Content.Create(ct).Description%></dd>
                <% } %> </dl> <%
                    } %>
