    ...
    <sn:ListHeaderCell  runat="server" ID="checkboxHeader" class="sn-lg-cbcol ui-state-default"><input type='checkbox' /></sn:ListHeaderCell>
      ...
      <sn:ListHeaderCell runat="server" class="sn-lg-col-4 sn-nowrap ui-state-default" FieldName="Task.AssignedTo" >      
        <asp:LinkButton runat="server" CommandName="Sort" CommandArgument="Task.AssignedTo" >
          <span class="sn-sort">
            <span class="sn-sort-asc ui-icon ui-icon-carat-1-n"></span>
            <span class="sn-sort-desc ui-icon ui-icon-carat-1-s"></span>
          </span>
          <span><%= ListHelper.GetColumnTitle("Task.AssignedTo", this.ContextNode) %></span>
        </asp:LinkButton>
      </sn:ListHeaderCell>
    
      <sn:ListHeaderCell runat="server" class="sn-lg-col-5 sn-nowrap ui-state-default" FieldName="Task.Status" >      
...
<script> $(function () { SN.Util.setFriendlyLocalDate('<%# "span.sn-date-" +
    ...
    System.Globalization.CultureInfo.CurrentUICulture.DateTimeFormat.ShortDatePattern.ToUpper() %>', <%# SenseNet.Portal.UI.UITools.DisplayTime(Container.DataItem, "DueDate").ToString().ToLower()%>); }); </script>
          ...
          <sn:ActionList runat='server' ActionName='Profile' ContentPathList='<%# ListHelper.GetPathList(Container.DataItem as SNCR.Content, "AssignedTo") %>' UseContentIcon="True" />    
